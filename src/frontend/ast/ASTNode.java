        ...
        if (!token.isTypeOf(expectedType) || !token.getContent().equals(expectValue)) {
            if (expectedType == TokenType.Semicolon) {
                ...
                ErrorCollector.getInstance().addError(
                        new CompileError(lineNum, ErrorType.MissSemicn, "got: " + token.getType())
                );
                return;
                ...
                int lineNum = tokens.prev().getLineNumber();
                ErrorCollector.getInstance().addError(
                        new CompileError(lineNum, ErrorType.MissRparent, "got: " + token.getType())
                );
                return;
                ...
                int lineNum = tokens.prev().getLineNumber();
                ErrorCollector.getInstance().addError(
                        new CompileError(lineNum, ErrorType.MissRbrack, "got: " + token.getType())
                );
                return;
            }
